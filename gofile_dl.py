...
Usage:
    python gofile_dl.py <gofile_url> [options]

Examples:
    python gofile_dl.py https://gofile.io/d/5tkZZi
    python gofile_dl.py https://gofile.io/d/5tkZZi --password MyPass
    python gofile_dl.py https://gofile.io/d/5tkZZi --proxy https://c.map987.dpdns.org/
    python gofile_dl.py https://gofile.io/d/5tkZZi -o /path/to/save
"""
...
def apply_proxy(url: str, proxy: str | None) -> str:
    ...
    The proxy works by prefixing: {proxy}{original_url}
    e.g. https://c.map987.dpdns.org/https://api.gofile.io/accounts
    """
    if proxy:
...
def download_file_with_resume(
    ...
    On connection failure, refreshes the download link and retries.
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        if final_path.exists() and final_path.stat().st_size > 0:
            ...
            try:
                ...
                if remote_size > 0 and file_size_on_disk == remote_size:
                    ...
                    # File on disk is larger than remote - corrupt, restart
                    log(f"  [WARN] {filename} on disk ({format_size(file_size_on_disk)}) > remote ({format_size(remote_size)}), restarting")
                    final_path.unlink()
            except Exception:
                pass  # HEAD not supported or failed, just try to resume
        ...
        try:
            ...
            # 416 = Range Not Satisfiable (file already complete or range invalid)
            ...
            if response.status_code not in (200, 206):
                ...
                if attempt < MAX_RETRIES:
                    # Try refreshing the download link
                    if content_id:
            ...
            if response.status_code == 206:
                # Partial content - Content-Range header tells us total
                ...
                # Full content (200) - server ignored Range header, start from scratch
                total_size = int(response.headers.get("Content-Length", -1))
                resume_from = 0  # restart from beginning
            ...
            if attempt < MAX_RETRIES:
                # Refresh download link before retrying
                if content_id:
                    new_link = refresh_download_link(content_id, filename, hashed_password, proxy)
                    if new_link:
                        download_link = new_link
...
def download(url: str, args: argparse.Namespace) -> None:
    ...
    # Step 7: Download files with resume support
...
Examples:
  ...
  %(prog)s https://gofile.io/d/5tkZZi --proxy https://c.map987.dpdns.org/
  %(prog)s https://gofile.io/d/5tkZZi -o /path/to/save
  %(prog)s https://gofile.io/d/5tkZZi --proxy https://c.map987.dpdns.org/ -w 16

Proxy usage:
  When GoFile is inaccessible (e.g. from China), use a Cloudflare Workers
  proxy. The proxy URL is prepended to the original URL:
    https://c.map987.dpdns.org/https://api.gofile.io/accounts
...
""",
    ...
    parser.add_argument(
        "--proxy",
        help="Cloudflare Workers proxy URL prefix (e.g. https://c.map987.dpdns.org/). "
